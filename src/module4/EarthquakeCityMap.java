...
//		text("Below 4.0", 75, 225);
		...
		// CityMarker
		float textYInit = 100; // same as y
		float textY = textYInit;
		
		float x = 50;
		float y = 100;
		float deltaX = 5;
		
		float ySymbol = textYInit;
		
		float x1 = x-deltaX;
		float x2 = x;
		float x3 = x+deltaX;
		float y1 = (float) (y+Math.sqrt(3.0)/3.0*deltaX);
		float y2 = (float) (y-2.0*Math.sqrt(3)/3.0*deltaX);
		float y3 = (float) (y+Math.sqrt(3)/3.0*deltaX);
		fill(color(102, 0, 0));
		triangle(x1, y1, x2, y2, x3, y3);
		ySymbol = ySymbol + 20;
		fill(color(255, 255, 255));
		ellipse(x, ySymbol, 10, 10);
		ySymbol = ySymbol + 20;
		fill(color(255, 255, 255));
		rectMode(CENTER);
		rect(x, ySymbol, 10, 10);
		//
		fill(0, 0, 0);
		text("City Marker", 75, textYInit);
		textY = textY + 20;
		text("Land Quake", 75, textY);
		textY = textY + 20;
		text("Ocean Quake", 75, textY);
		textY = textY + 20;
		text("Size ~ Magnitude", 50, textY);
...
//		text("X", x, ySymbol);
		...
		textSize(12);
		fill(0, 0, 0);
		text("Shallow", 75, textY);
		textY = textY + 20;
		text("Intermediate", 75, textY);
		textY = textY + 20;
		text("Deep", 75, textY);
		//
//		textY = textY + 20;
//		text("Past Hour", 75, textY);
	}
	...
	private void printQuakes() 
	{
		...
		eqCounts.put("OCEAN QUAKES", 0);
		// loop over all earthquakes to build the hashmap
		for (Marker quakeM : quakeMarkers) {
			EarthquakeMarker em = (EarthquakeMarker)quakeM;
			if (em.isOnLand()) {
				String countryName = (String)em.getProperty("country");
				if (eqCounts.containsKey(countryName)) {
					eqCounts.put(countryName, eqCounts.get(countryName)+1);
				}
				else {
					eqCounts.put(countryName, 1);
				}
			}
			else {
				eqCounts.put("OCEAN QUAKES", eqCounts.get("OCEAN QUAKES")+1);
			}
			
		}
		// print the elements in the hashmap
		for (String countryName : eqCounts.keySet()) {
			System.out.println(countryName + ": " + eqCounts.get(countryName));
		}
